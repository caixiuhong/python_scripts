...
#allows you to use '#' to comment out lines in the input files such as run.prm or head3.lst
def read_file_respect_comments(file):
...
def first_ph():
   ...
   lines=read_file_respect_comments("run.prm")
   for line in lines:
...
def read_headlst():
   ...
   lines = read_file_respect_comments("head3.lst")
   ...
   for line in lines:
      fields = line.split()
      conformer = CONFORMER()
      conformer.confid= int(fields[0]) -1
...
def read_fort38():
   ...
   lines = read_file_respect_comments("fort.38")
...
def read_pK():
   lines = read_file_respect_comments("pK.out")
...
def E_ionize(res_pKa_name):
   ...
   for conformer in residue[1]+residue[2]:
      ...
      if conformer.id.find("DM") > -1:
        ...
        #      else:
             # still add vdw+ele even vdw=999
           while (len(line[3]) > 8):
...
#         if Eref > conf.E_total[i]: Eref=conf.E_total[i]
   ...
   Eref   = [0.0 for i in range(len(titration_range))]      #change 'Eref' to an array to store all the Erefs, by xuyu 01/03/12
   for i in range(len(titration_range)):
      Eref[i] = residue[1][0].E_total[i]
      for conformer in residue[1]:
         if conformer.E_total[i] < Eref[i]: Eref[i] = conformer.E_total[i]
      Ei = [math.exp(-(conformer.E_total[i]-Eref[i])*Kcal2kT) for conformer in residue[1]]
   ...
   for conformer in residue[1]:
      ...
      for i in range(len(titration_range)):
         if conformer.fl == 'T' or conformer.fl == 't':
            ...
            conformer.rocc[i] = math.exp(-(conformer.E_total[i]-Eref[i])*Kcal2kT)/SigmaE[i]  # recovered occ
   ...
   if all0:
      for conformer in residue[1]:
         for i in range(len(titration_range)):
            conformer.rocc[i] = math.exp(-(conformer.E_total[i]-Eref[i])*Kcal2kT)/SigmaE[i]
   ...
   for conformer in residue[1]:
      conformer.nocc = []
      for i in range(len(titration_range)):
            # if all conformer.roo[i] = 0, then it's a problem
            # but mfe for partial fixed residue is not complete
            conformer.nocc.append(conformer.rocc[i]/SigmaOcc[i])

   # for conformer type 2
   SigmaE = [0.0 for i in range(len(titration_range))]
   Eref   = [0.0 for i in range(len(titration_range))]
   for i in range(len(titration_range)):
      Eref[i] = residue[2][0].E_total[i]
      for conformer in residue[2]:
         if conformer.E_total[i] < Eref[i]: Eref[i] = conformer.E_total[i]
      Ei = [math.exp(-(conformer.E_total[i]-Eref[i])*Kcal2kT) for conformer in residue[2]]
      for x in Ei: SigmaE[i] += x
   ...
   titr_point = int(t_point)
   print("extra conformer info of residue of interest at pH %d:" % titr_point)
   print("name\t\tself\t\tmfe_total\tnocc")
   for conformer in residue[1]+residue[2]:
      print("%s\t%f\t%f\t%f" % (conformer.id, conformer.self, conformer.mfe_total[0], conformer.nocc[0]))
...
def print_crg(RES):
   line = read_file_respect_comments('sum_crg.out')
   found=1
   for eachline in line:
      tok=eachline[10:].split()
      a = eachline[:10]      #change the format to read the residue name correctly in case there's a space in it, by xuyu
      # a=tok[0]
      a1=a[0:2]+a[4:9]
      b=RES[0:2]+RES[3:8]
      if (a1==b):
...
def run_mfe(ARGV1, ARGV2, ARGV3=-0.001):
   global ph1, eh1, scale_ele, scale_vdw, scale_vdw0, scale_vdw1, scale_tors, scale_dsolv,fname_extra,ph2Kcal, mev2Kcal,kal2KT,conformers,residues,titration_range,titration_type
   global t_point
   #Global variables and default values
   ph1         = 0.0
   eh1         = 0.0
   ...
   # print
   for x in pK:
      if (ARGV1[:3] == x[0][:3] and ARGV1[4:] == x[0][4:]):
         res_pKa = x[1].split()[0]
   print("Residue %s pKa/Em=%s" % (ARGV1, res_pKa))
   print("=================================")
   print("Terms          pH     meV    Kcal")
   print("---------------------------------")
   print("vdw0     %8.2f%8.2f%8.2f" % (dG_point.vdw0/ph2Kcal, dG_point.vdw0/mev2Kcal, dG_point.vdw0))
